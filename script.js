...
function Animation() {
    ...
    // hit the target
    if (range(smallX, x, x + side) || range(smallX + target, x, x + side)) { // X
        if (range(smallY, y, y + side) || range(smallY + target, y, y + side)) { // Y
            // make the target
            targetDisplay();
            radius = radius + 5;
            //Score increasing.
    ...
    if (counttime % 5 === 0) {
        ...
        var dangerY = 80;
        var sqr = 25;
        if (range(dangerX, x, x + side) || range(dangerX + sqr, x, x + sqr)) {
            if (range(dangerY, y, y + side) || range(dangerY + sqr, x, x + sqr)) {
                alert("Ball touch the ball");
                gameOver();
            }
        }
    }
